...
def _resize_volume_and_targets(self, volume: Tensor, target_size: int) -> Tensor:
    """Resize a 3D volume maintaining aspect ratio"""
    d, h, w = volume.shape[-3:]
    min_dim = min(d, h, w)
    scale_factor = target_size / min_dim
    
    # Use interpolate for 3D resizing
...
class Generalized3DRCNNTransform(nn.Module):
    """
    ...
    def forward(
        ...
        for i in range(len(images)):
            ...
            if image.dim() != 4:
                raise ValueError(f"Volumes are expected to be a list of 4d tensors of shape [C, D, H, W], got {image.shape}")
    ...
    def resize(
        ...
        volume, target = _resize_image_and_masks(volume, size, self.max_size, target, self.fixed_size)

        if target is None:
            return volume, target

        # bboxes stay 2D, hence we only need to scale height and width       
        bbox = target["boxes"]
        bbox = resize_boxes(bbox, (h, w), volume.shape[-2:])
        target["boxes"] = bbox

        if "keypoints" in target:
            keypoints = target["keypoints"]
            keypoints = resize_keypoints(keypoints, (h, w), volume.shape[-2:])
