...
def get_train_transforms_nifti(
    augment: bool,
    patch_size: Tuple[int, int, int],
    batch_size: int,
    image_key: str,
    ...
    if augment:
        train_transform = Compose([
            LoadImaged(keys=[image_key], image_only=False, meta_key_postfix="meta_dict", reader="NibabelReader"),
            Lambdad(keys=[image_key], func=lambda x: np.array(x)),
            EnsureChannelFirstd(keys=[image_key], channel_dim='no_channel'),
            EnsureTyped(keys=[image_key, box_key], dtype=torch.float32),
            EnsureTyped(keys=[label_key], dtype=torch.long),
